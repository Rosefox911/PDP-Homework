...
(define-struct ball (coordinates selected? offset))
;; A Ball is a (make-ball Posn Boolean Posn)
;; Interpretation:
...
(define BALL-1 (make-ball (make-posn 100 100) false INITIAL-OFFSET))
;; Ball that is unselected

(define BALL-2 (make-ball (make-posn 110 110) true  INITIAL-OFFSET))
;; Ball that is selected

(define BALL-3 (make-ball (make-posn 50 50) true  INITIAL-OFFSET))
;; Ball that is selected


(define WORLD-1 (make-world (list BALL-1 BALL-2 BALL-3)))
;; a world with BALL-1, BALL-2 and BALL-3
...
;; EXAMPLES: (run 0) -> Initiates the world
...
;; EXAMPLES: 
;; (world-on-tick empty) -> empty
;; STRATEGY: DOMAIN KNOWLEDGE
(define (world-on-tick world)
  world)

;; TESTS
(define-test-suite world-on-tick-tests
  (check-equal? 
   (world-on-tick empty) 
   empty) 
  "Testing to make sure same world is returned")
(run-tests world-on-tick-tests)
...
;; EXAMPLES: 
;; (initial-world 1) -> (make-world empty)
;; STRATEGY: FUNCTIONAL COMPOSITION
...
;; (cons (make-ball (make-posn 100 100) false (make-posn 0 0)) empty)))
;; STRATEGY: STRUCTURAL DECOMPOSITION on balls : ListOfBalls
...
(define (keep-unselected balls)
  (cond
    [(empty? balls) empty]
    [(cons? balls)
     (if (ball-selected? (first balls)) ; is it selected?
         (keep-unselected (rest balls)) ; yes - don't keep it
         (cons (first balls)            ; no  - keep and recurse
               (keep-unselected (rest balls))))]))

;; TESTS
(define-test-suite keep-unselected-tests
  (check-equal? (keep-unselected LOB-1)
                (cons 
                 (make-ball (make-posn 100 100) false (make-posn 0 0)) empty)))
...
(define (add-ball world)
  (make-world (cons (make-ball INITIAL-POSN false INITIAL-OFFSET)
                    (world-balls world))))
...
(define-test-suite add-ball-tests
  (check-equal? (add-ball WORLD-1) 
                (make-world
                 (cons
                  (make-ball (make-posn 200 150) false (make-posn 0 0))
                  (cons
                   (make-ball (make-posn 100 100) false (make-posn 0 0))
                   (cons 
                    (make-ball (make-posn 110 110) true (make-posn 0 0))
                    (cons 
...
(define-test-suite world-after-d-tests
  (check-equal? 
   ...
   (make-world (cons (make-ball 
                      (make-posn 100 100) false (make-posn 0 0)) empty))))
...
(define-test-suite world-after-key-event-tests
  (check-equal? (world-after-key-event WORLD-1 "n")
                (make-world
                 (cons
                  (make-ball 
                   (make-posn 200 150) false (make-posn 0 0))
                  (cons
                   (make-ball 
                    (make-posn 100 100) false (make-posn 0 0))
                   (cons 
                    (make-ball 
                     (make-posn 110 110) true (make-posn 0 0)) 
                    (cons 
                     (make-ball (make-posn 50 50) true 
                                (make-posn 0 0)) empty)))))
...
(define (select-balls x y balls)
  (cond
    [(empty? balls) empty]
    [(cons? balls)
     (cons (make-ball (ball-coordinates (first balls))
                      (in-circle? x y (first balls))
                      (calc-offset x y (ball-coordinates (first balls))))
           (select-balls x y (rest balls)))]))

;; TESTS
(define-test-suite select-balls-tests
  (check-equal? (select-balls 100 100 LOB-1)
                (cons
                 (make-ball (make-posn 100 100) true (make-posn 0 0))
                 (cons 
                  (make-ball (make-posn 110 110) true (make-posn 10 10)) 
                  (cons 
                   (make-ball (make-posn 50 50) false 
                              (make-posn -50 -50)) empty))) 
   ...
   (make-world
    (cons
     ...
     (cons 
      (make-ball (make-posn 110 110) true (make-posn 10 10)) 
      (cons 
       (make-ball (make-posn 50 50) false (make-posn -50 -50)) empty))))
...
(define (balls-after-drag x y balls)
  (cond
    [(empty? balls) empty]
    [(cons? balls)
     (cons (if (ball-selected? (first balls))
               (ball-after-drag x y (first balls))
               (first balls))
           (balls-after-drag x y (rest balls)))]))
...
(define-test-suite balls-after-drag-tests
  (check-equal? 
   ...
   (cons
    (make-ball (make-posn 100 100) false (make-posn 0 0))
    (cons 
     (make-ball (make-posn 120 120) true (make-posn 0 0)) 
     (cons 
      (make-ball (make-posn 120 120) true (make-posn 0 0)) empty)))
   ...
   (make-world
    (cons
     ...
     (cons 
      (make-ball (make-posn 100 100) true (make-posn 0 0)) 
      (cons 
       (make-ball (make-posn 100 100) true (make-posn 0 0)) empty))))
  ...
  (check-equal? 
   ...
   (make-world
    (cons
     (make-ball (make-posn 100 100) false (make-posn 0 0))
     (cons 
      (make-ball (make-posn 110 110) false (make-posn 0 0)) 
      (cons 
       (make-ball (make-posn 50 50) false (make-posn 0 0)) empty))))
...
(define-test-suite world-after-mouse-event-tests
  (check-equal? 
   ...
   (make-world
    (cons
     (make-ball (make-posn 100 100) false (make-posn 50 50))
     (cons 
      (make-ball (make-posn 110 110) false (make-posn 60 60)) 
      (cons                                                      
       (make-ball (make-posn 50 50) true 
                  (make-posn 0 0)) empty)))) 
  ...
  (check-equal? 
   ...
   (make-world
    (cons
     (make-ball (make-posn 100 100) false (make-posn 0 0))
     (cons 
      (make-ball (make-posn 60 60) true (make-posn 0 0)) 
      (cons 
